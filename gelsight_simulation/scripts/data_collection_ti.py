...
PKG_PATH = rospack.get_path('gelsight_simulation')
...
obj_name = PKG_PATH + "/objs/obj_" + args.particle + "/" + args.object + ".npy"
...
def initialize_taichi(obj: ti.types.ndarray(),num_obj: ti.f32, ind_x: ti.f32, ind_y: ti.f32):
    ...
    for i in ti.ndrange(int(num_obj)):
...
def timg_generation(img_name):
    ...
    npy_name = PKG_PATH + "/results/depth/"+ img_name +".npy"
    with open(npy_name, 'wb') as f:
        np.save(f, i_depth)

    img = generate(i_depth)
    img_name = PKG_PATH + "/results/sim/"  + img_name +".png"
    ...
    for x_ in range(-(x_steps // 2), (x_steps // 2) + 1):
        for y in range(-(y_steps // 2), (y_steps // 2) + 1):
            ...
            for z in range(z_steps + 1):
                ...
                name = str(solid) + '_' + '_' + str(k) + '_' + str(x_) + '_' + str(y) + '_' + str(z) + '.png'
